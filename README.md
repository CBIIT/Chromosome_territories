...
- 3D volume, overlap, contact, shape, centroid, radial-position, shell, and per-nucleus FISH signal-count measurements;
- simplified public result table with internal tuning/debug columns removed while keeping final P/Q FISH signal counts;
...
## Default parameters retained from v0.3.6
...
- number of final segmented P-arm and Q-arm FISH signal compartments per nucleus (`p_fish_signal_count`, `q_fish_signal_count`);
...
The collaborator-requested FISH signal counts are saved directly in the per-nucleus CSV:
...
These are the counts of disconnected 3D compartments in the final accepted P-arm and Q-arm masks for each nucleus, after GMM detection, presence gating, optional MRF/CRF refinement, morphology, and component selection. A value of `0` means no final signal was retained for that arm in that nucleus. A value of `1` means one connected signal was retained. Values greater than `1` indicate that the arm signal was retained as multiple separated compartments.

The population summary also includes total, mean, median, and positive-nucleus fractions for these signal-count columns.

---

## Plots

The plot set includes summary figures for arm volume fraction, P/Q overlap, contact frequency, FISH signal count per nucleus, minimum edge distance, P versus Q volume, mean arm volume versus overlap, centroid separation, radial centroid position, and shape/sphericity summaries when the required columns are present.
